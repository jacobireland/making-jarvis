...
5. **Auto-rearm** — after the spoken reply finishes, listening starts again (`voiceCursor.autoRearmListening`, default on). Click the status-bar item to turn Voice Cursor off. The status bar is the only listen indicator: **LISTENING** or **IDLE** (no notification toast).

## Flow
1. Click the status-bar **Voice Cursor** item to turn listening **on** (**LISTENING**)
2. Speak → **pause to send**, or status-bar click → **Send now**
3. Transcript → inject into Agent → spoken thoughts → spoken final reply
4. Listening **arms again** until you click to turn it **off** (**IDLE**)
...
| `voiceCursor.quietUi` | `true` | Skip routine success toasts; listen state is the status bar only |
...
- Pause-to-send does **not** listen during TTS (barge-in is still queued). After the reply, listening auto-rearms (`voiceCursor.autoRearmListening`). The Flux listen socket and PCM capture host stay **parked** across TTS so rearm does not wait to reconnect or respawn the recorder.
